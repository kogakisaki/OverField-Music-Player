...
| **🎹 MIDI 导入** | **新功能！** 通过内置 API 直接将 `.mid` 文件转换为 JSON（Beta）。 |
| **🔁 循环模式** | **新功能！** 自动重复当前歌曲。 |
...
- **Auto-Next (自动下曲)**: 自动播放库中的下一首歌曲。
- **Loop (循环播放)**: 歌曲结束时自动重播当前歌曲。
...
1. 在脚本界面点击 **🎹 Import MIDI (Beta)** 按钮。
2. 选择您的 `.mid` 或 `.midi` 文件。
3. 脚本将自动通过 API 进行转换，并询问您是否要保存到音乐库。

#### 方法 2：手动网页转换

如果内置导入器无法使用，您仍可以使用 **Tone.js MIDI** 工具：
...
- [x] **直接支持 MIDI**: 内置 MIDI 转 JSON 转换器 API (Beta)。
