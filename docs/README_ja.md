...
| **🎹 MIDI インポート** | **新機能！** 内蔵 API を介して `.mid` ファイルを JSON に直接変換します (Beta)。 |
| **🔁 ループモード** | **新機能！** 現在の曲を自動的に繰り返します。 |
...
- **プレイリスト**: ▲ / ▼ ボタンで曲順を並べ替え。
- **ナビゲーション**: **Next** / **Prev** ボタンで曲を切り替えます。
...
- **自動次曲再生**: ライブラリ内の次の曲を自動的に再生します。
- **ループ**: 現在の曲が終了すると自動的に再生します。
...
楽曲を準備するには 2 つの方法があります：

#### 方法 1: 内蔵インポーター (推奨)

1. スクリプト画面で **🎹 Import MIDI (Beta)** ボタンをクリックします。
2. `.mid` または `.midi` ファイルを選択します。
3. API を介して自動的に変換され、ライブラリに保存するかどうか確認されます。

#### 方法 2: 手動ウェブ変換

内蔵インポーターが利用できない場合は、**Tone.js MIDI** ツールを使用できます：
